...
@Component({
    ...
    template : `
                <h2>User Review:</h2>
                <textarea   [ngModel]="review_input" (ngModelChange)="valuechange($event)"
                  class="textfield" placeholder="Write your Review" [value]="review_input"  ></textarea>
                <br/><br/>
                <h3>Output:</h3>
...
At https://wallethub.com <b>bolded text</b>`
    ...
    ngOnInit() {
        this.review_content =  this.transform(this.review_input);
    }

    valuechange(newValue) {
        this.review_input = newValue;
        this.review_content = this.transform(this.review_input);
    }
