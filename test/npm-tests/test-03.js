...
(function () {
    ...
    const parsedData = data.replace(' ', '').split(',');

    // Use fs.createWriteStream() method 
    // to write the file 
    let writer = fs.createWriteStream('./npm-tests/output.txt');
    
    for (let index = 0; index < parsedData.length; index = index + 2) {
        const element = parsedData[index];
        newJson[element.replace('key=', '').replace(' ', '')] = Number(parsedData[index + 1].replace('age=', '').replace(' ', ''));
    }
    for (let index = 0; index < Object.keys(newJson).length; index++) {
        const elementFirst = Object.keys(newJson)[index];
        for (let j = index + 1; j < Object.keys(newJson).length; j++) {
            const element = Object.keys(newJson)[j];

            if (newJson[element] + newJson[elementFirst] === 32) {
                count += 1;
                result.push({ element: newJson[element] })
                writer.write(`key=${elementFirst},value=${newJson[elementFirst]}\r\n`);
                writer.write(`key=${element},value=${newJson[element]}\r\n`);
            }
        }
    }
